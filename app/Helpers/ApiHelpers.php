...
//        return response()->json(array("status" => $status, "data" => $data),200,['Content-Type'=>'application/json'],JSON_NUMERIC_CHECK);
    }
    ...
    public static function reBuildProviderCalendar(Request $request,$day, $calendar)
    {
        $dayDate = Carbon::parse($day)->format('Y-m-d');
        $morning = ["start" => "$dayDate 00:00:00", "end" => "$dayDate 12:00:00"];
        $afternoon = ["start" => "$dayDate 12:00:00", "end" => "$dayDate 17:00:00"];
        ...
        $reBuitCalendar->afternoon = [];
        $reBuitCalendar->evening = [];
        $calendar->sortBy('start_date')->each(function ($calendar) use ($morning, $afternoon, $evening, &$reBuitCalendar) {
            $carbonStartDate = Carbon::parse($calendar->start_date);
            $slot = new \stdClass();
            $slot->id = $calendar->id;
