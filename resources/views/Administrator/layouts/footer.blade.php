...
{{--    @include(ADL.'.sidebar')--}}
    ...
    <div class="page-footer">
        <div class="page-footer-inner"> {{ date('Y') }} &copy; Made by <i class="fa fa-heart text-danger"></i>
...
{{--    @include(ADL.'.quick-nav')--}}
    ...
    {{--lightbox--}}
    <script src="{{ asset('public/assets/global/scripts/lightbox.min.js')}}" type="text/javascript"></script>
...
{{--    <script src="{{ asset('public/assets/pages/scripts/components-editors.js')}}" type="text/javascript"></script>--}}
        ...
        <div class="modal-dialog modal-sm">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    {!! Form::open(['method'=>'POST','role'=>'form','url'=>AD.'/delete_select_admins', 'class'=>'form-horizontal', 'id'=>'Delete_selected_form']) !!}

                    {!! Form::hidden('select_ids','1',['class'=>'select_ids']) !!}
    ...
    <script type="text/javascript">
        $(".delete_button").click(function(){
            var id = $(this).data("id");
            var action = "{!! url(AD.'/'.Request::segment(2)) !!}"+"/"+id;
            $("#Delete_form").attr("action",action);
            $('html, body').animate({ scrollTop: 0 }, 0);
        });
    </script>

    <!-- Select All Checkbox And Delete Them -->
    <script>
        $(document).ready(function(){
            // check all
            $("#check_all").change(function () {
                $("input:checkbox").prop('checked', $(this).prop("checked"));
                var values = getCheckboxsValues();
            });

            // Delete All Selected
            $('.delete_all_selected').click(function(){
                var Values = getCheckboxsValues();
                $(".select_ids").val(Values);
                var action = "{!! url(AD.'/delete_select_'.Request::segment(2)) !!}";
                $("#Delete_selected_form").attr("action",action);
            });

            // Get All Selected
            function getCheckboxsValues() {
                var checkedValues = $('.All_Checkbox:checked').map(function() {
                    return this.value;
                }).get();
            ...
            function disabled()
            {
                $("#delete_all_selected").attr("disabled","disabled");
            }
            function removeDisabled()
            {
                $("#delete_all_selected").removeAttr("disabled");
            }
        });
    ...
    <script>
        var ComponentsEditors = function () {
            ...
            var handleWysihtml5 = function () {
                ...
                if ($('.wysihtml5').size() > 0) {
                    $('.wysihtml5').wysihtml5({
                        "stylesheets": ["{{ asset('public/assets/global/plugins/bootstrap-wysihtml5/wysiwyg-color.css') }}"]
                    });
                }
            }
            return {
